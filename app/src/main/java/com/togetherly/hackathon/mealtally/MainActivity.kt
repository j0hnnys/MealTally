...
class MainActivity : AppCompatActivity() {
    ...
    private fun setListeners(enableButtons: Boolean) {
        ...
        mealsFromVendor.onFocusChange { view, focus ->
            if (!focus) {
                calculateTotalMeals()
            }
        }
        mealsLeftover.onFocusChange { view, focus ->
            if (!focus) {
                calculateTotalMeals()
            }
        }

//        mealsLeftover.addTextChangedListener(textChangeListener())
//        mealsFromVendor.addTextChangedListener(textChangeListener())
    ...
    private fun updateMealCounts() {
        ...
        mealCountForm.vendorReceived = meals_from_vendor.text.toString() ?: "0"
        mealCountForm.carryOver = meals_leftOver.text.toString() ?: "0"
    }
    ...
    fun textChangeListener() = object: TextWatcher {
    ...
    fun calculateTotalMeals() {
        val mealsVendor = if (mealsFromVendor.text.toString().isEmpty()) "0" else mealsFromVendor.text.toString()
        val mealsCarryOver = if (mealsLeftover.text.toString().isEmpty()) "0" else mealsLeftover.text.toString()
